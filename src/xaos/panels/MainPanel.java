...
public final class MainPanel {
	...
	public static void toggle3DMouse() {
		...
		// // Game.getCurrentTask ().setPointIni (new Point3D (p3dIni.x, p3dIni.y,
		// getMaxZ3DMouse (p3dIni.x, p3dIni.y, p3dIni.z)));
		// Game.getCurrentTask ().setPointIni (getMax3DMouse (p3dIni.x, p3dIni.y,
		// p3dIni.z));
		// }
		// }
	}
	...
	public static void render() {
		...
		if (bMouseInMainArea) {
			int iTextureID = renderAllTerrains(zView, cellXMin, cellXMax, cellYMin, cellYMax, iBaseXGeneral,
					iBaseYGeneral, pointTileMouse, 0, -1);
			iTextureID = renderAllEntities(zView, cellXMin, cellXMax, cellYMin, cellYMax, iBaseXGeneral, iBaseYGeneral,
					pointTileMouse, 0, iTextureID);
			iTextureID = renderMouse(iBaseXGeneral, iBaseYGeneral, zView, pointTileMouse, iTextureID);
			renderTask(zView, iBaseXGeneral, iBaseYGeneral + Tile.TERRAIN_ICON_HEIGHT, pointTileMouse, iTextureID,
					cellXMin, cellYMin);
		} else {
			int iTextureID = renderAllTerrains(zView, cellXMin, cellXMax, cellYMin, cellYMax, iBaseXGeneral,
					iBaseYGeneral, null, 0, -1);
			iTextureID = renderAllEntities(zView, cellXMin, cellXMax, cellYMin, cellYMax, iBaseXGeneral, iBaseYGeneral,
					null, 0, iTextureID);
			renderTask(zView, iBaseXGeneral, iBaseYGeneral + Tile.TERRAIN_ICON_HEIGHT, null, iTextureID, cellXMin,
					cellYMin);
		}
					...
					if (cell.hasZone()) {
						...
						if (zone != null) {
							...
							if (zView < (World.MAP_DEPTH - 1)) {
								...
								currentTextureID = setColorShadowLightCellTerrain(cellDown, tile, zLevelOffset + 1,
	...
	public static int setColorShadowLightCellTerrain(Cell cell, Tile tile, int zLevelOffset, int iCurrentTextureID,
		...
		if (cell.isLight() && Game.OPENGL_13_AVAILABLE) {
			...
			return setColorShadowLightCellTerrain(cell, getColorShadowLightCell(cell, zLevelOffset), tile,
					bTransparency, iCurrentTextureID, zLevelOffset);
		}
	}

	public static int setColorShadowLightCell(Cell cell, Tile tile, int zLevelOffset, int iCurrentTextureID,
			boolean bTransparency) {
		if (cell.isLight() && Game.OPENGL_13_AVAILABLE) {
			return setColorShadowLightCell(cell, 0, tile, bTransparency, iCurrentTextureID, zLevelOffset);
		} else {
			return setColorShadowLightCell(cell, getColorShadowLightCell(cell, zLevelOffset), tile, bTransparency,
					iCurrentTextureID, zLevelOffset);
		}
	}

	public static int setColorShadowLightCellNoLight(Cell cell, Tile tile, int zLevelOffset, int iCurrentTextureID,
			boolean bTransparency) {
		return setColorShadowLightCellNoLight(cell, getColorShadowLightCell(cell, zLevelOffset), tile, bTransparency,
				iCurrentTextureID);
	}

	public static int setColorShadowLightCell(Cell cell, float fShadowLightColor, Tile tile, boolean bTransparency,
			int iCurrentTextureID, int zLevelOffset) {
		if (cell.isLight()) {
			if (Game.OPENGL_13_AVAILABLE) {
				...
				if (cell.isOpen()) {
					...
					if (bTransparency) {
						GL11.glColor4f(fRed + ged.getRed(), fGreen + ged.getGreen(), fBlue + ged.getBlue(),
								TRANSPARENCY);
					} else {
			...
			GlobalEventData ged = Game.getWorld().getGlobalEvents();
			if (bTransparency) {
				GL11.glColor4f(fShadowLightColor + ged.getRed(), fShadowLightColor + ged.getGreen(),
						fShadowLightColor + ged.getBlue(), TRANSPARENCY);
			} else {
				GL11.glColor3f(fShadowLightColor + ged.getRed(), fShadowLightColor + ged.getGreen(),
						fShadowLightColor + ged.getBlue());
			}
			return iCurrentTextureID;
		}
	}

	public static int setColorShadowLightCellTerrain(Cell cell, float fShadowLightColor, Tile tile,
			boolean bTransparency, int iCurrentTextureID, int zLevelOffset) {
		if (cell.isLight()) {
			if (Game.OPENGL_13_AVAILABLE) {
				if (zLevelOffset == 0) {
	...
	public static int setColorShadowLightCellNoLight(Cell cell, float fShadowLightColor, Tile tile,
			boolean bTransparency, int iCurrentTextureID) {
		iCurrentTextureID = UtilsGL.setTexture(tile, iCurrentTextureID);
	...
	public static int getMaxZ3DMouse(int x, int y, int z) {
		...
		while (iZ < World.MAP_DEPTH && cell.isMined() && !cell.getTerrain().hasFluids()
				&& (imi == null || !imi.isBase())) {
			iZ++;
			if (iZ < World.MAP_DEPTH) {
	...
	public static void renderTask(int zView, int iBaseXGeneral, int iBaseYGeneral, Point3D pointTileMouse,
			int currentTextureID, int cellXMin, int cellYMin) {
		// Tasks
		if (pointTileMouse != null) {
			if (Game.getCurrentState() == Game.STATE_CREATING_TASK
				...
				int iXGeneral, iYGeneral;
				int z3D;
				if (tDMouseON && Game.getCurrentState() == Game.STATE_CREATING_TASK
						&& Game.getCurrentTask().getState() == Task.STATE_CREATING_ENDZONE) {
				...
				// Miramos donde está el ratón para mostrar info por pantalla y el cuadradito
				// blanco
				...
				if (Game.getCurrentTask().getState() == Task.STATE_CREATING_ENDZONE) {
					if (Game.getCurrentTask().getTask() == Task.TASK_STOCKPILE) {
						...
						zmi = ZoneManager.getItem(expandZone.getIniHeader()); // Usamos el expandzone solo por
																				// "performance"
					} else if (Game.getCurrentTask().getTask() == Task.TASK_CREATE_AND_PLACE_ROW
							|| Game.getCurrentTask().getTask() == Task.TASK_QUEUE_AND_PLACE_ROW) {
						rowing = true;
						bIteming = true;
						...
						bIteming = true;
						queuing = true;
					} else if (Game.getCurrentTask().getTask() == Task.TASK_MINE
							|| Game.getCurrentTask().getTask() == Task.TASK_MINE_LADDER) {
						mining = true;
					...
					if (Game.getCurrentTask().getTask() == Task.TASK_BUILD
							|| Game.getCurrentTask().getTask() == Task.TASK_CREATE_AND_PLACE
							|| Game.getCurrentTask().getTask() == Task.TASK_QUEUE_AND_PLACE) {
						// Building
						if (Game.getCurrentTask().getTask() == Task.TASK_BUILD) {
							bBuilding = true;
						} else {
						...
						Log.log(Log.LEVEL_ERROR,
								Messages.getString("MainPanel.3") + Game.getCurrentTask().getTask() + "]", "MainPanel"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
					}
				}
				...
				if (bBuilding) {
					...
					if (imi == null) {
						// No ha encontrado el item??? Quizá es un createItemByType y el valor es un
						// type, así que pillamos el primer item con ese type
						if (queuing) {
							// En caso de colas el item está en el parámetro 2
				...
				if (zoning) {
					// bZoneOK = Zone.areCellsAvailableForZone (zmi, x1, y1, x2, y2, zView,
					// expandZone);
						...
						if (mouseOK) {
							...
							if (bBuilding) {
								if (bBuildingOK) {
									...
									char groundDataChar = bmi.getGroundData()
											.charAt((j - y1) * bmi.getWidth() + (i - x1));
								...
								if (!Item.isCellAvailableForItem(imi, i, j, z3D, true, true)) {
									...
									// El item puede ir ahí, pero miraremos que no esté construyendo en el aire y no
									// haya camino posible
							...
							if (bDraw) {
								...
								iXGeneral = iBaseXGeneral + (i + j) * (Tile.TERRAIN_ICON_WIDTH / 2)
										+ terrainSpecialTile.getTileWidthOffset();
								iYGeneral = iBaseYGeneral - (i - j) * (Tile.TERRAIN_ICON_HEIGHT / 2)
										- terrainSpecialTile.getTileHeight() - terrainSpecialTile.getTileHeightOffset()
										+ Tile.TERRAIN_ICON_HEIGHT;

								currentTextureID = UtilsGL.setTexture(terrainSpecialTile, currentTextureID);
								UtilsGL.drawTextureZ(iXGeneral, iYGeneral + (z3D - zView) * Tile.TERRAIN_ICON_HEIGHT,
										iXGeneral + terrainSpecialTile.getTileWidth(),
										iYGeneral + terrainSpecialTile.getTileHeight()
												+ (z3D - zView) * Tile.TERRAIN_ICON_HEIGHT,
										terrainSpecialTile.getTileSetTexX0(), terrainSpecialTile.getTileSetTexY0(),
										terrainSpecialTile.getTileSetTexX1(), terrainSpecialTile.getTileSetTexY1(),
										Cell.getDepth(i, j, z3D));
								...
								if (bIteming && !bUseMouseBad && imi != null && imi.isCanBeRotated()) {
									...
									iXGeneral = iBaseXGeneral + (i + j) * (Tile.TERRAIN_ICON_WIDTH / 2)
											+ terrainSpecialTile.getTileWidthOffset();
									iYGeneral = iBaseYGeneral - (i - j) * (Tile.TERRAIN_ICON_HEIGHT / 2)
											- terrainSpecialTile.getTileHeight()
											- terrainSpecialTile.getTileHeightOffset() + Tile.TERRAIN_ICON_HEIGHT;

									currentTextureID = UtilsGL.setTexture(terrainSpecialTile, currentTextureID);
									UtilsGL.drawTextureZ(iXGeneral,
											iYGeneral + (z3D - zView) * Tile.TERRAIN_ICON_HEIGHT,
											iXGeneral + terrainSpecialTile.getTileWidth(),
											iYGeneral + terrainSpecialTile.getTileHeight()
													+ (z3D - zView) * Tile.TERRAIN_ICON_HEIGHT,
											terrainSpecialTile.getTileSetTexX0(), terrainSpecialTile.getTileSetTexY0(),
	...
	private static int renderEntities(int i, int j, int zView, Cell cell, int iXGeneral, int iYGeneral,
		...
		if (cell.isDiscovered()) {
			if (cell.isMined()) {
				...
				// Items
				Item item = cell.getItem();
				currentTextureID = renderItems(cell, item, iXGeneral, iYGeneral, currentTextureID, pointTileMouse,
						fColorShadowLight, iDepth, zLevelOffset);
				...
				// Livings
				currentTextureID = renderLivings(cell, iXGeneral, iYGeneral, currentTextureID, fColorShadowLight,
						bLight, zLevelOffset);
			}
		}
	...
	private static boolean isMouseNearCell(Cell cell, Point3D pointTileMouse) {
		return (Math.abs(cell.getCoordinates().x - pointTileMouse.x) <= 3
				&& Math.abs(cell.getCoordinates().y - pointTileMouse.y) <= 3);
	}

	private static boolean isMouseNearCell(Cell cell, Point3D pointTileMouse, Tile tile) {
		return (Math.abs(cell.getCoordinates().x - pointTileMouse.x) <= 3
				&& Math.abs(cell.getCoordinates().y - pointTileMouse.y) <= 3
				&& tile.getTileHeight() > Tile.TERRAIN_ICON_HEIGHT);
	}
	...
	private static int renderLivings(Cell cell, int iXGeneral, int iYGeneral, int currentTextureID,
		...
		for (int liv = 0; liv < alLivings.size(); liv++) {
			...
			if (le.getPath().size() == 0) {
				...
				if (p3d.x > cell.getCoordinates().x && p3d.y > cell.getCoordinates().y) {
					iDepth = Math.max(
							...
							Cell.getDepth(p3d.x - 1, p3d.y, p3d.z));
				} else if (p3d.x < cell.getCoordinates().x && p3d.y < cell.getCoordinates().y) {
					iDepth = Math.max(
							Cell.getDepth(cell.getCoordinates().x, cell.getCoordinates().y, cell.getCoordinates().z),
							Cell.getDepth(p3d.x, p3d.y + 1, p3d.z));
				} else {
					iDepth = Math.max(
							Cell.getDepth(cell.getCoordinates().x, cell.getCoordinates().y, cell.getCoordinates().z),
							Cell.getDepth(p3d.x, p3d.y, p3d.z));
				}
			}

			// Si la celda tiene un item grande, miraremos que el iDepth no tenga que ser
			// mayor
			...
			// Skills / Effects icon
			// Haremos 8 turnos mostrando, 8 sin mostrar, 8 mostrando ..... alternando entre
			// efectos
	...
	private static int renderHero(Cell cell, Hero hero, int iXGeneral, int iYGeneral, int currentTextureID,
		...
		if (!bGraphicChanged) {
			if (iFacingDirection == LivingEntity.FACING_DIRECTION_NORTH
					|| iFacingDirection == LivingEntity.FACING_DIRECTION_NORTH_EAST
					|| iFacingDirection == LivingEntity.FACING_DIRECTION_EAST) {
				currentTextureID = renderCarrying(cell, hero, iXGeneral, iYGeneral, currentTextureID, fColorShadowLight,
						bLight, iDepth, zLevelOffset);
			}
		}
	...
	private static void renderTooltip(int x, int y, Point3D pointTileMouse) {
		...
		// int iZ3D;
		// if (tDMouseON) {
		// // iZ3D = getMaxZ3DMouse (pointTileMouse.x, pointTileMouse.y,
		// pointTileMouse.z);
		// // cell = World.getCell (pointTileMouse.x, pointTileMouse.y, iZ3D);
		// cell = World.getCell (getMax3DMouse (pointTileMouse.x, pointTileMouse.y,
		// pointTileMouse.z));
			...
			if (cell.hasZone()) {
				...
				if (cell.getTerrain().hasFluids()) {
					...
					if (cell.isMined() && pointTileMouse.z < (World.MAP_DEPTH - 1)) {
						sMessage = TerrainManager
								.getItemByID(World.getCell(pointTileMouse.x, pointTileMouse.y, pointTileMouse.z + 1)
										.getTerrain().getTerrainID())
								.getName();
					} else {
				...
				for (int i = 0; i < alLivings.size(); i++) {
					...
					if (lemi.getType() == LivingEntity.TYPE_CITIZEN) {
						...
						if (cit.getSoldierData().isSoldier()
								&& cit.getSoldierData().getState() == SoldierData.STATE_IN_A_GROUP
								&& cit.getSoldierData().getGroup() >= 0
								&& cit.getSoldierData().getGroup() < SoldierGroups.MAX_GROUPS) {
							alMessages.add(cit.getCitizenData().getFullName() + " (" //$NON-NLS-1$
									+ cit.getLivingEntityData().getHealthStatus() + ") (" + Game.getWorld() //$NON-NLS-1$
											.getSoldierGroups().getGroup(cit.getSoldierData().getGroup()).getName()
									+ ")"); //$NON-NLS-1$
							alColor.add(ColorGL.YELLOW);
						} else {
							// Grupo
							if (cit.getCitizenData().getGroupID() != -1 && Game.getWorld().getCitizenGroups()
									.getGroup(cit.getCitizenData().getGroupID()) != null) {
								alMessages.add(cit.getCitizenData().getFullName() + " (" //$NON-NLS-1$
										+ cit.getLivingEntityData().getHealthStatus() + ") (" //$NON-NLS-1$
										+ Game.getWorld().getCitizenGroups().getGroup(cit.getCitizenData().getGroupID())
												.getName()
										+ ")"); // );
							} else {
								alMessages.add(cit.getCitizenData().getFullName() + " (" //$NON-NLS-1$
						...
						// Level / Xp
						if (cit.getSoldierData().isSoldier()) {
							alMessages.add(Messages.getString("Hero.4") + cit.getSoldierData().getLevel() + " (" //$NON-NLS-1$ //$NON-NLS-2$
									+ cit.getSoldierData().getXp() + Messages.getString("Hero.5") //$NON-NLS-1$
									+ cit.getSoldierData().getXpPCT() + "%)"); //$NON-NLS-1$
							alColor.add(ColorGL.WHITE);
						}

						if (TownsProperties.DEBUG_MODE) {
							alMessages.add("Hap/HWork/HIdle: " + cit.getCitizenData().getHappiness() + "/"
									+ cit.getCitizenData().getHappinessWorkCounter() + "/"
									+ cit.getCitizenData().getHappinessIdleCounter());
							alColor.add(ColorGL.YELLOW);
						}
					} else if (lemi.getType() == LivingEntity.TYPE_HERO) {
						Hero hero = (Hero) le;
						...
						alMessages.add(hero.getCitizenData().getFullName() + " (" //$NON-NLS-1$
								+ le.getLivingEntityData().getHealthStatus() + ")"); //$NON-NLS-1$ );
						alColor.add(ColorGL.YELLOW);
						...
						if (lemi.getCaravan() != null) {
							...
							alMessages.add(le.getLivingEntityData().getName() + " (" //$NON-NLS-1$
									+ le.getLivingEntityData().getHealthStatus() + ")"); //$NON-NLS-1$
						}
						alColor.add(ColorGL.YELLOW);
					}
		...
		// alMessages.add ("x " + cell.getCoordinates ().x + " y " + cell.getCoordinates
		// ().y + " z " + cell.getCoordinates ().z + " XMin " + depthXMin + " YMin " +
		// depthYMin + " Depth " + cell.getDepth ());
		// alColor.add (ColorGL.ORANGE);
	...
	public void mousePressed(int x, int y, int mouseButton) {
		...
		if (Game.getCurrentState() == Game.STATE_CREATING_TASK) {
			// if (tDMouseON) {
			// //Game.getCurrentTask ().setPoint (new Point3D (p3d.x, p3d.y, getMaxZ3DMouse
			// (p3d.x, p3d.y, p3d.z)));
	...
	private static Point3D getTileMouse(int x, int y, int xView, int yView, int zView) {
		...
		if (tDMouseON) {
			...
			if (pointTileMouse.x < 0 || pointTileMouse.x >= World.MAP_WIDTH || pointTileMouse.y < 0
					|| pointTileMouse.y >= World.MAP_HEIGHT) {
				return null;
			}
		}
	...
	public ContextMenu getContextMenu(int x, int y) {
		...
		if (TownsProperties.DEBUG_MODE) {
			sm.addItem(
					new SmartMenu(SmartMenu.TYPE_TEXT, "A*ZI: " + cell.getAstarZoneID(), null, null, null, null, p3d)); //$NON-NLS-1$
			sm.addItem(
					new SmartMenu(SmartMenu.TYPE_TEXT, "Coord: " + cell.getCoordinates(), null, null, null, null, p3d)); //$NON-NLS-1$
		}

		if (sm.getItems().size() > 0) {
			sm.addItem(new SmartMenu(SmartMenu.TYPE_ITEM, Messages.getString("MainPanel.57"), null, //$NON-NLS-1$
					CommandPanel.COMMAND_CLOSE_CONTEXT, null));
	 ...
	 * @param iTexture
	 * 
	 *                 private static void unsetBrightNess (int iTexture) {
	 *                 //UtilsGL.glEnd (); // GL11.glBindTexture
	 *                 (GL11.GL_TEXTURE_2D, iTexture); // GL11.glTexEnvf
	 *                 (GL11.GL_TEXTURE_ENV, GL11.GL_TEXTURE_ENV_MODE,
	 *                 GL11.GL_MODULATE); // GL11.glTexEnvf (GL11.GL_TEXTURE_ENV,
	 *                 GL13.GL_COMBINE_RGB, GL11.GL_MODULATE); //UtilsGL.glBegin
	 *                 (GL11.GL_QUADS); }
	 */
	public static void resize(int renderWidth, int renderHeight) {
		...
		yCentro = ((renderHeight / 2) / Tile.TERRAIN_ICON_HEIGHT) * Tile.TERRAIN_ICON_HEIGHT
				- (Tile.TERRAIN_ICON_HEIGHT / 2);
