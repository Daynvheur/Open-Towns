...
public final class Game {
	...
	public Game() {
		...
		panelUI = new UIPanel();
		if (getWorld() == null) {
			panelCommand = new CommandPanel(UtilsGL.getWidth() - World.MAP_WIDTH, World.MAP_HEIGHT, World.MAP_WIDTH,
					UtilsGL.getHeight() - World.MAP_HEIGHT, null, null);
		} else {
			panelCommand = new CommandPanel(UtilsGL.getWidth() - World.MAP_WIDTH, World.MAP_HEIGHT, World.MAP_WIDTH,
		...
		// Esto aquí, después de cargar las texturas, que si no empieza el contador del
	...
	public static void continueGame(String savegameName, String sCampaignMissionAndForcedZipPath) {
		...
		if (bForzedZip) {
			...
			fMapa = new File(Game.getUserFolder() + Game.getFileSeparator() + Game.SAVE_FOLDER1
					+ Game.getFileSeparator() + savegameName); // $NON-NLS-1$
		}
		...
		if (!bNewGame) {
			...
			try {
				if (bForzedZip) {
					...
					Utils.load(Game.getUserFolder() + Game.getFileSeparator() + Game.SAVE_FOLDER1
							+ Game.getFileSeparator(), fMapa.getName()); // $NON-NLS-1$
				}
	...
	public static void startGame(String campaignID, String missionID) {
		...
		if (campaignID == null || missionID == null) {
			fMapa = new File(Game.getUserFolder() + Game.getFileSeparator() + Game.SAVE_FOLDER1
					+ Game.getFileSeparator() + "freemode.zip"); //$NON-NLS-1$ //$NON-NLS-2$
		} else {
			fMapa = new File(Game.getUserFolder() + Game.getFileSeparator() + Game.SAVE_FOLDER1
					+ Game.getFileSeparator() + campaignID + missionID + ".zip"); //$NON-NLS-1$ //$NON-NLS-2$
		}
		if (fMapa.exists()) {
			fMapa.delete();
		}
	...
	private static void setupGame(boolean bNewGame, String sCampaignID, String sMissionID, boolean bForzedZip) {
			...
			if (le != null) {
				...
				if (lemi != null && lemi.getType() == LivingEntity.TYPE_CITIZEN) {
					...
					if (cit.getCitizenData() != null && cit.getCitizenData().getGroupID() == -1) {
						// Ciudadano sin grupo (pq no tiene grupo, pq es un save de la v10 o pq es una
						// nueva partida)
		...
		// Esto es un parche para solventar un bug. Antes no se borraban los ciudadanos
		// de los job grupos al morir, y podría estar corrupto
				...
				if (missionData == null) {
					if (bNewGame) {
						Log.log(Log.LEVEL_ERROR,
								Messages.getString("Game.14") + " [" + sCampaignID + "][" + sMissionID + "]", "Game"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
						Game.exit();
						// } else {
						// // Cargando una partida y no encuentra la campaña, ponemos todo vacío y
						// pacasa
						// MissionData md = new MissionData(""); //$NON-NLS-1$
						// missionPanel.setMissionData(md);
					}
					// } else {
					// MessagesPanel.addMessage (MessagesPanel.TYPE_SYSTEM, Messages.getString
					// ("Game.5"), ColorGL.GREEN); //$NON-NLS-1$
				}
				// if (missionData != null && missionPanel.getMissionData().getText() != null &&
				// missionPanel.getMissionData().getObjectives() != null &&
				// missionPanel.getMissionData().getObjectives().size() > 0) {
				// MessagesPanel.addMessage(MessagesPanel.TYPE_SYSTEM,
				// Messages.getString("Game.0") + CampaignManager.getMission(sCampaignID,
				// sMissionID).getName() + "]", ColorGL.GREEN); //$NON-NLS-1$ //$NON-NLS-2$
				// MessagesPanel.addMessage(MessagesPanel.TYPE_SYSTEM,
				// Messages.getString("Game.5"), ColorGL.GREEN); //$NON-NLS-1$
				// }
			} else {
				missionData = null;
			}
		...
		if (bNewGame) {
			if (World.getCitizenIDs().size() > 0) {
				...
				if (le != null) {
					...
					Game.getWorld().setView(Point3DShort.getPoolInstance(World.MAP_WIDTH / 2, World.MAP_HEIGHT / 2,
							World.MAP_NUM_LEVELS_OUTSIDE - 1));
				}
...
public static void taskCreated(Task task) {
	...
	private void checkMouseEvents() {
		...
		while (Mouse.next()) {
			...
			if (Mouse.getEventButtonState()) {
				...
				// if (mouseX >= getCurrentInfoPanel().getX() && mouseX <
				// (getCurrentInfoPanel().getX() + getCurrentInfoPanel().getWidth()) && mouseY
				// >= getCurrentInfoPanel().getY() && mouseY < (getCurrentInfoPanel().getY() +
				// getCurrentInfoPanel().getHeight())) {
				// getCurrentInfoPanel().mousePressed(mouseX - getCurrentInfoPanel().getX(),
				// mouseY - getCurrentInfoPanel().getY());
				// }
				// continue;
				// }

				if (mouseButton == 0) {
					// Botón izquierdo pulsado, miramos a qué panel pertenece y llamamos a su
					// función mousePressed (x, y) (con x e y relativas al panel)
					...
					if (getCurrentState() == STATE_SHOWING_CONTEXT_MENU) {
						...
						if (mouseX >= getCurrentContextMenu().getX()
							...
							getCurrentContextMenu().mousePressed(mouseX - getCurrentContextMenu().getX(),
									mouseY - getCurrentContextMenu().getY());
						} else {
							// Cierra el menú
							deleteCurrentContextMenu();
					...
					if (typingPanel != null) {
						...
						if (mouseX >= 0 && mouseX < MainPanel.renderWidth && mouseY >= 0
							...
							// } else if (mouseX >= getPanelMessages ().renderX && mouseX <
							// (getPanelMessages ().renderX + getPanelMessages ().renderWidth) && mouseY >=
							// getPanelMessages ().renderY && mouseY < (getPanelMessages ().renderY +
							// getPanelMessages ().renderHeight)) {
							// Messages panel
							// setContextMenu (getPanelMessages ().getContextMenu (mouseX - getPanelMessages
							// ().renderX, mouseY - getPanelMessages ().renderY));
						}
					}
				}
			}
					...
					if (Game.isMouseScrollEarsON() || !isMouseCloseToOpenCloseProductionIcon(mouseX, mouseY)) {
						if (getCurrentState() == STATE_SHOWING_CONTEXT_MENU) {
							if (!(mouseX >= getCurrentContextMenu().getX()
									...
									&& mouseY >= getCurrentContextMenu().getY()
									&& mouseY < (getCurrentContextMenu().getY()
											+ getCurrentContextMenu().getHeight()))) {
			...
			if (Keyboard.getEventKeyState()) {
				...
				if (typingPanel != null) {
					...
					if (iKEY == Keyboard.KEY_ESCAPE && getCurrentState() == STATE_SHOWING_CONTEXT_MENU) {
						...
						// } else if (iKEY == Keyboard.KEY_ESCAPE && getCurrentState() ==
						// STATE_SHOWING_INFO_PANEL) {
						// closeInfoPanel();
					} else {
						// Función
						int iFN = UtilsKeyboard.getFN(iKEY);
						...
						if (iFN == UtilsKeyboard.FN_SHOW_MISSION) {
							...
							// if (getCurrentState() != STATE_SHOWING_INFO_PANEL) {
							// // Panel de misión
							// MissionPanel panel = new MissionPanel(getWorld().getCampaignID(),
							// getWorld().getMissionID());
							// if (getCurrentMissionData () != null && getCurrentMissionData ().getText() !=
							// null && getCurrentMissionData ().getObjectives() != null &&
							// getCurrentMissionData ().getObjectives().size() > 0) {
							// showInfoPanel(panel);
							// if (UIPanel.imagesPanel == null) {
							// UIPanel.imagesPanel = new ImagesPanel (MainPanel.renderWidth,
							// MainPanel.renderHeight, getCurrentMissionData ().getTutorialImages ());
							// }
							// } else {
							// continue;
							// }
							...
							if (World.SPEED > 1) {
								...
								MessagesPanel.addMessage(MessagesPanel.TYPE_SYSTEM,
										Messages.getString("Game.6") + World.SPEED, ColorGL.YELLOW); //$NON-NLS-1$
							}
	...
	public void run() {
		...
		// int checkTrigger = (4 * FPS_INGAME); // Para que lo mire nada más empezar, en
		// el primer ciclo
		// boolean bVictory = false;

		// int iMissionCompletedWidth = (UtilFont.getWidth (Messages.getString
		// ("Game.2"))) / 2; //$NON-NLS-1$
		while (!Display.isCloseRequested()) {
			...
			if (!getPanelMainMenu().isActive()) {
				world.nextTurn();
				/*
				 * // Comprobamos la victoria cada 4xFPS_INGAME turnos (para ganar un poquillo
				 * de performance)
				 * if (bVictory == false && world != null && getWorld ().getCampaignID () !=
				 * null) {
				 * checkVictory++;
				 * if (checkVictory >= (4 * FPS_INGAME)) {
				 * checkVictory = 0;
				 * MissionData md = CampaignManager.getMission (getWorld ().getCampaignID (),
				 * getWorld ().getMissionID ());
				 ...
				 * checkTrigger = 0;
				 * if (Game.getCurrentMissionData () != null && Game.getCurrentMissionData
				 * ().getTutorialFlowIndex () < Game.getCurrentMissionData ().getTutorialFlows
				 * ().size ()) {
				 * if (Game.getCurrentMissionData ().getTutorialFlows ().get
				 * (Game.getCurrentMissionData ().getTutorialFlowIndex ()).getTriggers () !=
				 * null && Game.getCurrentMissionData ().getTutorialFlows ().get
				 * (Game.getCurrentMissionData ().getTutorialFlowIndex ()).getTriggers ().size
				 * () > 0) {
				 * // There are triggers to check
				 * if (TutorialTrigger.checkCompleted (Game.getCurrentMissionData
				 * ().getTutorialFlows ().get (Game.getCurrentMissionData
				 * ().getTutorialFlowIndex ()).getTriggers ().get (0))) {
				 * // Trigger completed!
				 * Game.getCurrentMissionData ().getTutorialFlows ().get
				 * (Game.getCurrentMissionData ().getTutorialFlowIndex ()).getTriggers ().remove
				 * (0);
				 * // If no more triggers we update the index
				 * if (Game.getCurrentMissionData ().getTutorialFlows ().get
				 * (Game.getCurrentMissionData ().getTutorialFlowIndex ()).getTriggers ().size
				 * () == 0) {
				 * Game.getCurrentMissionData ().setTutorialFlowIndex
				 * (Game.getCurrentMissionData ().getTutorialFlowIndex () + 1);
				 * }
				 * 
			...
			// }
			// fps++;
			// Pongo este método pq dice la doc de LWJGL que actualiza los timers y tiene
			// que llamarse lo más posible
			Sys.getTimerResolution();

			// Render
			// if (getPanelMainMenu ().isActive () || (world != null && World.getCitizenIDs
			// ().size () == 0)) {
			render();
			 ...
			 * UtilsGL.drawStringWithBorder (Messages.getString ("Game.2"),
			 * (MainPanel.renderWidth / 2) - iMissionCompletedWidth, MainPanel.renderHeight
			 * / 2 - UtilFont.MAX_HEIGHT / 2, ColorGL.RED, ColorGL.WHITE); //$NON-NLS-1$
			 * UtilsGL.glEnd ();
			 * 
			 * if (getCurrentState () == STATE_SHOWING_CONTEXT_MENU && getCurrentContextMenu
			 * () != null) {
			 * // ContextMenu
	...
	public static void setBlinkingCells(int iFlowIndex, boolean bValue) {
		if (getCurrentMissionData() != null && iFlowIndex >= 0
				&& iFlowIndex < Game.getCurrentMissionData().getTutorialFlows().size()) {
			// Check the triggers
			...
			if (flow.getTriggers() != null) {
				...
				for (int i = 0; i < flow.getTriggers().size(); i++) {
					if (flow.getTriggers().get(i).getType() == TutorialTrigger.TYPE_INT_PLACE) {
						...
						if (p3d != null && p3d.x >= 0 && p3d.x < World.MAP_WIDTH && p3d.y >= 0
								&& p3d.y < World.MAP_HEIGHT && p3d.z >= 0 && p3d.z < World.MAP_DEPTH) {
	...
	public static void updateTutorialFlow(int iTutorialType, int iParam, Point3D p3d, String sParam,
			boolean bAdvanceIfOk) {
		if (missionData != null && missionData.getTutorialFlowIndex() < missionData.getTutorialFlows().size()
				&& missionData.getTutorialFlowIndex() == ImagesPanel.getCurrentFlowIndex()) {
					...
					if (iTutorialType == trigger.getType()) {
						...
						switch (iTutorialType) {
							...
							case TutorialTrigger.TYPE_INT_WASD:
								...
								if (trigger.getParam2() >= (TutorialTrigger.WASD_UP + TutorialTrigger.WASD_DOWN
										+ TutorialTrigger.WASD_LEFT + TutorialTrigger.WASD_RIGHT)) {
									bTriggerEnd = true;
								}
							...
							case TutorialTrigger.TYPE_INT_ICONHIT:
								if (iParam == TutorialTrigger.ICON_INT_FLAT) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_FLAT));
								} else if (iParam == TutorialTrigger.ICON_INT_GRID) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_GRID));
								} else if (iParam == TutorialTrigger.ICON_INT_FLATCURSOR) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_FLATCURSOR));
								} else if (iParam == TutorialTrigger.ICON_INT_3DMOUSE) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_3DMOUSE));
								} else if (iParam == TutorialTrigger.ICON_INT_SETTINGS) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_SETTINGS));
								} else if (iParam == TutorialTrigger.ICON_INT_LEVELUP) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_LEVELUP));
								} else if (iParam == TutorialTrigger.ICON_INT_LEVELDOWN) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_LEVELDOWN));
								} else if (iParam == TutorialTrigger.ICON_INT_SPEEDUP) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_SPEEDUP));
								} else if (iParam == TutorialTrigger.ICON_INT_SPEEDDOWN) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_SPEEDDOWN));
								} else if (iParam == TutorialTrigger.ICON_INT_PAUSE) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_PAUSE));
								} else if (iParam == TutorialTrigger.ICON_INT_CITIZENS) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_CITIZENS));
								} else if (iParam == TutorialTrigger.ICON_INT_SOLDIERS) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_SOLDIERS));
								} else if (iParam == TutorialTrigger.ICON_INT_HEROES) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_HEROES));
								} else if (iParam == TutorialTrigger.ICON_INT_TRADE) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_TRADE));
								} else if (iParam == TutorialTrigger.ICON_INT_LIVINGS_LIVINGS) {
									bTriggerEnd = (trigger.getParam1() != null && trigger.getParam1()
											.equalsIgnoreCase(TutorialTrigger.ICON_LIVINGS_LIVINGS));
								} else if (iParam == TutorialTrigger.ICON_INT_LIVINGS_BODY) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_LIVINGS_BODY));
								} else if (iParam == TutorialTrigger.ICON_INT_LIVINGS_AUTOEQUIP) {
									bTriggerEnd = (trigger.getParam1() != null && trigger.getParam1()
											.equalsIgnoreCase(TutorialTrigger.ICON_LIVINGS_AUTOEQUIP));
								} else if (iParam == TutorialTrigger.ICON_INT_LIVINGS_CONVERTSOLDIER) {
									bTriggerEnd = (trigger.getParam1() != null && trigger.getParam1()
											.equalsIgnoreCase(TutorialTrigger.ICON_LIVINGS_CONVERTSOLDIER));
								} else if (iParam == TutorialTrigger.ICON_INT_LIVINGS_JOBS) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_LIVINGS_JOBS));
								} else if (iParam == TutorialTrigger.ICON_INT_LIVINGS_GROUPS) {
									bTriggerEnd = (trigger.getParam1() != null && trigger.getParam1()
											.equalsIgnoreCase(TutorialTrigger.ICON_LIVINGS_GROUPS));
								} else if (iParam == TutorialTrigger.ICON_INT_LIVINGS_CONVERTCIVILIAN) {
									bTriggerEnd = (trigger.getParam1() != null && trigger.getParam1()
											.equalsIgnoreCase(TutorialTrigger.ICON_LIVINGS_CONVERTCIVILIAN));
								} else if (iParam == TutorialTrigger.ICON_INT_LIVINGS_RESTRICTION) {
									bTriggerEnd = (trigger.getParam1() != null && trigger.getParam1()
											.equalsIgnoreCase(TutorialTrigger.ICON_LIVINGS_RESTRICTION));
								} else if (iParam == TutorialTrigger.ICON_INT_LIVINGS_GUARD) {
									bTriggerEnd = (trigger.getParam1() != null && trigger.getParam1()
											.equalsIgnoreCase(TutorialTrigger.ICON_LIVINGS_GUARD));
								} else if (iParam == TutorialTrigger.ICON_INT_LIVINGS_PATROL) {
									bTriggerEnd = (trigger.getParam1() != null && trigger.getParam1()
											.equalsIgnoreCase(TutorialTrigger.ICON_LIVINGS_PATROL));
								} else if (iParam == TutorialTrigger.ICON_INT_LIVINGS_BOSS) {
									bTriggerEnd = (trigger.getParam1() != null
											&& trigger.getParam1().equalsIgnoreCase(TutorialTrigger.ICON_LIVINGS_BOSS));
							...
							case TutorialTrigger.TYPE_INT_PLACE:
								if (p3d != null && trigger.getParamXYZ() != null) {
									if (UtilsIniHeaders.getIntIniHeader(trigger.getParam1()) == iParam) {
										if (p3d.x == trigger.getParamXYZ().x && p3d.y == trigger.getParamXYZ().y
												&& p3d.z == trigger.getParamXYZ().z) {
											bTriggerEnd = true;
						...
						// Here I'll check for NONE triggers and delete the PLACE ones if the items are
						// already set
						if (iTutorialType == TutorialTrigger.TYPE_INT_NONE
								&& trigger.getType() == TutorialTrigger.TYPE_INT_PLACE) {
							if (trigger.getParamXYZ() != null) {
								// We check the cell
								Cell cell = World.getCell(trigger.getParamXYZ());
								Item item = cell.getItem();
								if (item != null && item.getNumericIniHeader() == UtilsIniHeaders
										.getIntIniHeader(trigger.getParam1())) {
									// Bingo !
	...
	private void handleResize() {
		if (Display.wasResized() || Display.isFullscreen() != displayFullscreen) {
			...
			if (Game.getWorld() == null) {
				...
				Game.getPanelUI().resize(iWidth, iHeight, Game.getWorld().getCampaignID(),
						Game.getWorld().getMissionID(), true);
			}

			Game.getPanelCommand().resize(iWidth - World.MAP_WIDTH, World.MAP_HEIGHT, World.MAP_WIDTH,
					iHeight - World.MAP_HEIGHT);
	...
	private static void takeScreenshot() {
		...
		int iNumber = 1;
		while (!bNameFound) {
			file = new File(Game.getUserFolder() + Game.getFileSeparator() + Game.SCREENSHOTS_FOLDER1
					+ Game.getFileSeparator() + "Towns_" + iNumber + ".png"); //$NON-NLS-1$ //$NON-NLS-2$
			if (file.exists()) {
				iNumber++;
			} else {
				bNameFound = true;
